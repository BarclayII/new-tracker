...
class ConvLSTMCell(nn.Module):
    ...
    @staticmethod
    def init_hidden(batch_size, hidden_c, shape, use_cuda):
        if use_cuda:
...
class ConvLSTM(nn.Module):
    ...
    def __init__(self, input_channels, hidden_channels, kernel_size, step=1, effective_step=[1], bias=True, use_cuda=False):
        ...
        for step in range(self.step):
            ...
            for i in range(self.num_layers):
                ...
                if step == 0:
                    ...
                    (h, c) = ConvLSTMCell.init_hidden(bsize, self.hidden_channels[i], (height, width),self.use_cuda)
